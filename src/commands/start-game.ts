...
export async function startGame(message) {
	...
	.then((message) => {
		...
		for(let tist of gameData.hypnotists) {
			tist.send(`The game has started ! you are a hypnotist, along with ${gameData.hypnotists.map(h => h.username).join(", ")}`)
		}
		gameConfig.channel.send(`${gameConfig.allPlayers.map(p => `<@${p.id}>`).join(" , ")} , the game has started ! There's ${n} hypnotists in the town of spirals.`,);
		handleNight();
	});
}