...
export async function setBreak(message: Message, targetName: string) {
	...
	await message.author.send(`You go to ${targetNickname}'s house. While repeating to yourself that it's for the greater good, you use all your knowledge of conditioning to thoroughly break ${targetNickname}'s mind.`);
	await message.author.send(`Leaving ${targetNickname} behind as comfortably as possible, you know that despite everything the hypnotists do you won't be able to do that again.`);
...
export async function setSave(message: Message) {
	...
	const targetNickname = await getNickname(target);
	await message.author.send(`You go to ${targetNickname}'s house, where they appear to be sleeping peacefully - but you know what really is up. You set up your gear and spent an hour un-conditioning them.`);
	await message.author.send(`Unfortunately, your gear break just as ${targetNickname} shuffles to wake up. You take what you can and leave hurriedly, but you won't be able to un-condition anybody else for a while.`);
