...
export async function handleSpy(message: Message, targetName: string) {
	...
	if(gameConfig.hypnotists.some(h => h === target)) {
		await message.author.send(`They're not in, and you can see them coming back after a while - carrying out a weird device. You're pretty sure ${targetName} is a hypnotist.`);
	}
	else if(gameConfig.specials[target.id] === "detective") {
		await message.author.send(`They're not in, and you can see them snooping around another house. Could ${targetName} be a fellow detective ?`);
	}
	else if(gameConfig.specials[target.id] === "deprogrammer") {
		await message.author.send(`Rhe lights in their home are still on. ${targetName} is working on a special device, and you find notes in their bin about "breaking conditioning". You're pretty sure they're a deprogrammer.`);		
	} else {
