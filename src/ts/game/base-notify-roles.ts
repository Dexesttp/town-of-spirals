...
export function baseNotifyRoles(flavour: NotifyFlavour) {
  return (context: GameContext, tools: GameTools) =>
    Promise.all(
      context.players.map((p) => {
        ...
        const getMessage =
          (flavour.roles ? flavour.roles[p.roles.join("_")] : undefined) ||
          flavour.unknown ||
          ((playerInt: PlayerData, roleListInt: string[]) =>
            `A new game has started ! You are a ${p.roles.join(", ")}.`);
        return context.playerInterface[p.id].sendMessage(
          getMessage(p, p.roles)
        );
      })
    )
      .then((_) => {
        const getStartMessage =
          flavour.start ||
          ((playerList: PlayerData[], hypnotistCount: number) =>
            `Game started ! There is ${hypnotistCount} hypnotists`);
