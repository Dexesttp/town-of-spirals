...
export function baseResolveAllBroken(flavour: ResolveBrokenFlavour) {
  return async (context: GameContext, internalsTools: GameTools) => {
    const getIntro =
      flavour.intro || (() => `A new day rises on the town of spirals.`);
    await context.sendMessage(getIntro());

    const brokenPlayers = context.players.filter((p) =>
      p.attributes.some((a) => a === BROKEN_NIGHT)
    );
    if (brokenPlayers.length === 0) {
      const getNoBroken =
        flavour.noBroken || (() => `Nobody got broken tonight.`);
      await context.sendMessage(getNoBroken());
      return;
    }
    const potentialOwners = GetAlivePlayers(context).filter(
    ...
    for (const player of brokenPlayers) {
      ...
      if (potentialOwners.length > 0) {
        const getBroken =
          flavour.broken ||
          ((target: PlayerData, ownerInt: PlayerData) =>
            `${player.nickname} has been broken tonight.`);
        const owner = getRandom(potentialOwners, 1)[0];
        await context.sendMessage(getBroken(player, owner));
      } else {
        const getNoOwnerLeft =
          flavour.noOwnerLeft ||
          ((target: PlayerData) =>
            `${player.nickname} has been broken tonight.`);
      ...
      // Reveals
      if (!context.reveal_roles) continue;
      ...
      const getRole =
        (flavour.roles ? flavour.roles[player.roles.join("_")] : undefined) ||
        flavour.unknown ||
        ((target: PlayerData, roleList: string[]) =>
          `<@${target.id}> was a ${roleList.join(", ")}`);
