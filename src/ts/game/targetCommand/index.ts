...
import {
  InputTargetType,
  TargettingCommandData,
  TargettingHandler,
  TargettingPromiseGetter,
} from "./types";
...
export function targetEngine(
  ...
  function subscribeForTargetting(
    ...
    if (!handleTargettingOnce[command]) handleTargettingOnce[command] = [];
  ...
  async function handleTargettingResult(
    ...
    switch (result) {
      case TargetRequestResult.INVALID_MODE:
        if (message.private) {
          playerInterface[voterID].sendMessage(
            "You must use this command publicly."
          );
          return;
        }
        await tryDeleteMessage(message);
        playerInterface[voterID].sendMessage(
          "Do not use commands in public chats by night ! Please do it here via PMs."
        );
        return;
      ...
      case TargetRequestResult.INVALID_TARGET:
        if (message.private) {
          playerInterface[voterID].sendMessage(
            `Invalid command ${message.content} : this player isn't a valid target.`
          );
          return;
        }
    ...
    return new Promise<TargettingCommandData>((resolve, reject) => {
      subscribeForTargetting(command, (playerID, target, message: Message) => {
        ...
        const actualTarget = (
          target.type === "id"
            ? targets.filter(
                (t) => target.type === "id" && t.id === target.content
              )
            : targets.filter(
                (t, index) =>
                  target.type === "index" && index === target.content
              )
        )[0];
        if (!actualTarget) {
          reject(TargetRequestResult.INVALID_TARGET);
  ...
  function handleTargettingCommand(
    ...
    if (!handleTargettingOnce[command]) return;
