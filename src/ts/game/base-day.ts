...
export function baseDay(flavour: DayFlavour) {
  return async (context: GameContext, tools: GameTools) => {
    const voteList = GetAlivePlayers(context).map(
      (p, i) => `[${i + 1}] ${p.nickname} (${p.username})`
    );
    ...
    if (
      ...
      const targetPlayer = context.players.filter(
        (p) => p.id === voteResult.target
      )[0];
      ...
      const getRole =
        (flavour.roles
          ? flavour.roles[targetPlayer.roles.join("_")]
          : undefined) ||
        flavour.unknown ||
    ...
    if (
      ...
      const getNoVoteFlavour =
        flavour.noVote || (() => `Nobody was broken today.`);
      await context.sendMessage(getNoVoteFlavour());
      return;
    }
    const getDissentFlavour =
      flavour.dissent || (() => `Nobody was broken today.`);
