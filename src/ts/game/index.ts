...
export type GameData = {
  ...
  setDay: (
    newDay: (context: GameContext, tools: GameTools) => Promise<void>
  ) => void;
  setNight: (
    newNight: (context: GameContext, tools: GameTools) => Promise<void>
  ) => void;
  setCheckEnd: (
    newCheckEnd: (context: GameContext, tools: GameTools) => Promise<boolean>
  ) => void;
  setNotifyRoles: (
    newNotifyRoles: (context: GameContext, tools: GameTools) => Promise<void>
  ) => void;
  setResolveAllBroken: (
    newResolveAllBroken: (
      context: GameContext,
  ...
  subscribeNightRole: (
    role: (context: GameContext, internalTools: GameTools) => Promise<any>
  ) => void;
...
export function Game(
  ...
  const { cleanSubscribedCommands, getCommandPromise, handleCommand } =
  ...
  function* votePromiseGenerator(
    ...
    while (true) {
      yield Promise.race([
        ...
        getCommandPromise("no-vote", voters, shouldBePrivate).then((r) => {
          cleanSubscribedCommands();
          cleanSubscribedTargettingCommands();
  ...
  const nightRoles: Array<
    (context: GameContext, tools: GameTools) => Promise<any>
  > = [];
  let notifyRoles: (context: GameContext, tools: GameTools) => Promise<void> =
    baseNotifyRoles({});
  let day: (context: GameContext, tools: GameTools) => Promise<void> = baseDay(
    {}
  );
  let night: (context: GameContext, tools: GameTools) => Promise<void> =
    baseNight({});
  let checkEnd: (context: GameContext, tools: GameTools) => Promise<boolean> =
    baseCheckEnd({});
  let resolveAllBroken: (
    context: GameContext,
    tools: GameTools
  ) => Promise<void> = baseResolveAllBroken({});
  ...
  return {
    ...
    subscribeNightRole(
      role: (context: GameContext, tools: GameTools) => Promise<any>
    ) {
    ...
    async start() {
      ...
      while (true) {
        ...
        if (await checkEnd(context, internalsTools)) break;
        isDay = true;
