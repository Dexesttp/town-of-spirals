...
export function startVoteFactory(
  ...
  voteGenerator: (
    ...
    Promise<{ voterID: string; targetID: string | null | undefined }>
  >,
  sendMessage: (
    message: string,
    voters: PlayerData[],
  ...
  return function startVote(config: VoteConfig): Promise<VoteResult> {
    ...
    if (fullTimeout < warnTimeout)
      throw new TypeError(
        `The vote timeout ${fullTimeout} is less than the warn timeout ${warnTimeout}`
      );
    const flavours = config.flavour || {};
    const warnTimeoutTextGetter =
      flavours.onWarnTimeout ||
      ((timedOutVoters: PlayerData[]) =>
        `${timedOutVoters.map((p) => `<@${p.id}>`).join(", ")} : ${moment
          .duration(warnTimeout)
          .humanize()} remaining before vote ends !`);
    const voteTextGetter =
      flavours.onVote ||
      ((voter: PlayerData, target: PlayerData) =>
        `${voter.nickname} voted for ${target.nickname} !`);
    const noVoteTextGetter =
      flavours.onNoVote ||
    ...
    const currentVotesTextGetter =
      ...
      ((results) =>
        `Current votes are : ${results
          .map((r) => {
            if (r.target === null) return `\`No vote\` (${r.count})`;
    ...
    return new Promise((resolve, reject) => {
      ...
      timeout
        .then(() => {
          if (ended) return;
          const votes = GetResults(
            config.voters.map((v) => v.id),
            targets.map((t) => t.id),
            votingData
          );
          const timedOutVoters = config.voters.filter((v) =>
            votes.timedout.some((t) => t === v.id)
          );
          sendMessage(
            // tslint:disable-next-line:max-line-length
      ...
      const iterator = voteGenerator(
        config.voters,
        targets,
        !!config.sendDirectMessage
      );
      const handleVote = (reply?: {
        voterID: string;
        ...
        if (reply) {
          ...
          if (voter === null) {
            reject(
              new Error(`The voter ${reply.voterID} is not in the voters list.`)
            );
            return;
          }
          if (reply.targetID) {
            ...
            if (!target) {
              reject(
                new Error(
                  `The target ${reply.targetID} is not in the target list.`
                )
              );
              return;
            }
          ...
          if (voteResult.timedout.length === 0) {
            ...
            try {
              ...
              const results: Array<{
                ...
                count: number;
              }> = GetVoteResults(votingData).map((v) => {
                if (v.userID === null) {
                ...
                if (!target) {
                  throw new Error(
                    `Vote target ${v.userID} is not a valid target.`
                  );
                }
