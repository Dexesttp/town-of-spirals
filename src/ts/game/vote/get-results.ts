  ...
  for (let value in votingData) {
    if (votingData.hasOwnProperty(value)) {
      ...
      const actualTargetID =
        riggedTargetID === undefined ? targetID : riggedTargetID;
      const targetValues = results.filter((v) => v.userID === actualTargetID);
      if (targetValues.length > 0) targetValues[0].count += 1;
      else results.push({ userID: actualTargetID, count: 1 });
    }
  }
