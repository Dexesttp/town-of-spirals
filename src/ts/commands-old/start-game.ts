...
export async function startGame(message: Message) {
    ...
    if (message.author !== gameConfig.gameStarter) {
        ...
        gameConfig.channel.send(
            ...
            `This game was created by ${authorNickname}. They should start the game, or you should use the \`${CANCEL_CREATE_COMMAND}\` command to cancel the game.`,
        );
        return;
    }
    ...
    .then((m: Message) => {
        ...
        for (let player of gameConfig.allPlayers) {
            if (gameConfig.hypnotists.some(h => h === player)) {
                const hypnotistNames = gameConfig.hypnotists.filter(h => h.id !== player.id).map(h => h.username);
                if (hypnotistNames.length) {
                    player.send(`The game has started ! you are a hypnotist, along with ${hypnotistNames.join(", ")}`);
                }
                else {
                    player.send(`The game has started ! You're the only hypnotist in town.`);
                }
            }
