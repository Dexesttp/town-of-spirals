...
import { VOTE_COMMAND, SPY_COMMAND, SAVE_COMMAND, SKIP_COMMAND, BREAK_COMMAND, VOTE_NB_COMMAND } from "../constants";
import { timerA } from "./check-timer";
import { User } from "discord.js";
import { handleDay } from "./handle-day";
import { setDetective } from "../commands-old/spy";
import { canBreak, canSave, setDeprogrammer } from "../commands-old/deprogram";
import * as moment from "moment";

let remainingSpecialRoles: Array<{user: User, role: SpecialRole}> = [];
...
As a reminder, your colleagues are : ${saneTists.map(v => v.username).join(", ")}.
    ...
    for (let userID in gameConfig.specials) {
        if (gameConfig.specials.hasOwnProperty(userID)) {
...
export async function handleSpecialRole() {
    ...
    if (special.role === "deprogrammer") {
        ...
        console.log(
            ...
            `[${moment().format("YYYY-MM-DD HH:mm:ss")}] Deprogrammer time ! Remaining save : ${saveAllowed}, remaining break: ${breakAllowed}`,
        );
        ...
        if (breakAllowed)
            special.user.send(
                // tslint:disable-next-line:max-line-length
                `You can use the \`${BREAK_COMMAND}\` and your knowledge of conditioning to break somebody arbitrarily. Possible targets are ${alivePlayers.join(", ")}`,
            );
