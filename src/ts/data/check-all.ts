    ...
    if (voters.length > 0) {
        ...
        for (let tist of voters)
            tist.send(
                ...
                `You sneak to ${targetNickname}'s house. Carefully, without waking them up, you whisper them into trance, where you start the process of breaking their mind.`
            );
        return;
    }
    if (voters.some(t => t === target)) {
        if (voters.length === 2) {
            ...
            for (let tist of voters)
                tist.send(
                    // tslint:disable-next-line:max-line-length
                    `${tistNick} meets ${targetNickname} at their house, where they trick them into trance. ${targetNickname}'s mind quickly gets melted.`
                );
            return;
        }
        for (let tist of voters.filter(v => v !== target))
            tist.send(
                // tslint:disable-next-line:max-line-length
                `You and the other hypnotists sneak into ${targetNickname}'s house. With the ${voters.length - 1} of you, you restrain ${targetNickname} and begin the process of breaking their mind.`
            );
        target.send(
            // tslint:disable-next-line:max-line-length
            `A bit surprised, you get thrown on your bed by an angry-looking team of ${voters.length - 1} hypnotists. Before you can scream, one of them shut your mouth with their hand, and all you can do is listen to their words and feel your mind slipping away.`
        );
        return;
    }
    for (let tist of voters)
        tist.send(
            `You sneak to ${targetNickname}'s house, where you find your fellow tists. With their help, you break ${targetNickname}'s mind.`
        );
    return;
}
