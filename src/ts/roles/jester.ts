...
export function statsWithJester() {
  return (context: GameContext): GameResult => {
    const brokenJesterDay = context.players.filter(
      (p) =>
        p.roles.some((r) => r === JESTER_ROLE) &&
        p.attributes.some((a) => a === BROKEN_DAY)
    );
    ...
    return context.players.map((p) => ({
      id: p.id,
      alive:
        p.attributes.some((a) => a === BROKEN || a === BROKEN_NIGHT) &&
        !p.roles.some((r) => r === JESTER_ROLE),
      role: p.roles.join(", "),
    }));
  };
...
export function checkEndWithJester(
  ...
  return async (context: GameContext, tools: GameTools) => {
    ...
    const tistCount = alivePlayers.filter((p) =>
      p.roles.some((r) => r === HYPNOTIST_ROLE)
    ).length;
    const hypnotists = context.players.filter((p) =>
      p.roles.some((r) => r === HYPNOTIST_ROLE)
    );
    const brokenJesterDay = context.players.filter(
      (p) =>
    ...
    if (brokenJesterDay.length > 0) {
      const getJesterVictory =
        jesterFlavour.jester ||
        ((count: number) =>
          (
            playerList: PlayerData[],
            allPlayerList: PlayerData[],
            hypnotistList: PlayerData[]
          ) =>
            "Jester won !");
      await context.sendMessage(
        getJesterVictory(brokenJesterDay.length)(
          brokenJesterDay,
          context.players,
          hypnotists
        )
    ...
    if (alivePlayers.length === 0) {
      const getNoVictory =
        flavour.nobody ||
        ((allPlayerList: PlayerData[], hypnotistList: PlayerData[]) =>
          "Nobody won !");
      await context.sendMessage(getNoVictory(context.players, hypnotists));
      return true;
    }
    if (tistCount === 0) {
      const getTownVictory =
        flavour.town ||
        ((count: number) =>
          (
            playerList: PlayerData[],
            ...
            "Townspeople won !");
      await context.sendMessage(
        getTownVictory(alivePlayers.length)(
          alivePlayers,
          context.players,
          hypnotists
        )
      ...
      const getHypnotistsVictory =
        ...
        ((count: number) =>
          (
            ...
            "Hypnotists won !");
      await context.sendMessage(
        getHypnotistsVictory(alivePlayers.length)(
          alivePlayers,
          context.players,
          hypnotists
        )
