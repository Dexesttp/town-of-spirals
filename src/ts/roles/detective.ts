...
export function handleDetective(
  ...
  return async (context: GameContext, tools: GameTools) => {
    const detectives = GetAlivePlayers(context).filter((p) =>
      p.roles.some((r) => r === DETECTIVE_ROLE)
    );
    ...
    for (const detective of detectives) {
      ...
      //#region Timeout
      const timeoutPromise = TimerPromise(timeout).then<DetectiveCommandResult>(
        (r) => ({ command: "timeout" })
      );
      ...
      if (result.command === "spy") {
        ...
        const getSpyFlavour =
          (flavour.spy
            ? flavour.spy[roles.join("_") || "none"] || flavour.spy["unknown"]
            : undefined) ||
          ((targetInt: PlayerData, roleListInt: string[]) =>
            `${targetInt.nickname} is a ${roleListInt.join(", ")}`);
