...
export function createGame(context: ManagerContext) {
  return async (message: discord.Message) => {
    ...
    if (channel.type !== "GUILD_TEXT") {
      await channel.send(
        "You cannot create a game here. Go to a server channel where the bot is enabled."
      );
      return;
    }
    ...
    if (data === null) {
      await channel.send(
        "The ToS bot is not enabled for this channel. Ask an admin to register it."
      );
      return;
    }
    ...
    if (data.type === "RUNNING") {
      await channel.send(
        "A game is currently in progress. You cannot create a game while another is running."
      );
      return;
    }
    ...
    if (!player) {
      logger.channel(
        ...
        `There was an error (unk 103) : player who created lobby couldn't be added to game`
      );
      return;
    }
    logger.channel(
      newData.channel.name,
      `Lobby created by ${message.author.username}.`
    );
    channel.send(
