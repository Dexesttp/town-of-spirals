...
export function updateStatsInternal(
  ...
  for (const player of results) {
    ...
    if (player.alive) playerData.wins += 1;
    else playerData.losses += 1;
    if (!playerData.roles[player.role]) playerData.roles[player.role] = 0;
...
export function getLeaderboardInternal(stats: StatsData) {
  return Object.keys(stats.saved)
    ...
    .map((s) => ({
      id: s,
      name: stats.names[s].nickname,
      wins: stats.saved[s].wins,
    }))
...
export function getUserStatsInternal(
  stats: StatsData,
  text: string,
  defaultId: string
): UserStatsResult {
  ...
  const idList = Object.keys(stats.saved).filter((key) => {
  ...
  const resultList = idList
    .map(getFromID)
    .filter((r) => !!r) as UserStatsResultEntry[];
